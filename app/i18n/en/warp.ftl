# Narwarp Desktop — English (source-of-truth locale)
...
common-skip = Skip
common-get-warping = Get Narwarping
...
agent-management-source-local-agent = Narwarp (local agent)
agent-management-source-cloud-agent = Tusk
...
workspace-menu-update-warp-manually = Update Narwarp manually
...
workspace-menu-view-warp-logs = View Narwarp logs
...
app-menu-set-default-terminal = Set Narwarp as Default Terminal
app-menu-file = File
app-menu-edit = Edit
app-menu-use-warp-prompt = Use Narwarp's Prompt
...
app-menu-help = Help
app-menu-warp-documentation = Narwarp Documentation...
app-menu-github-issues = GitHub Issues...
app-menu-warp-slack-community = Narwarp Slack Community...
workspace-update-and-relaunch-warp = Update and relaunch Narwarp
workspace-updating-to-version = Updating to ({ $version })
workspace-update-warp-manually = Update Narwarp manually
...
terminal-banner-shell-config-incompatible = Your shell configuration is incompatible with Narwarp...{"  "}
...
ssh-remote-choice-install-extension = Install Narwarp's SSH extension
ssh-remote-choice-install-extension-desc = Install Narwarp's extension to enable agent features like file browsing, code review, and intelligent command completions in this session.
...
ai-document-save-and-sync-tooltip = Save and auto-sync this plan to your Narwarp Drive
ai-document-show-in-warp-drive = Show in Narwarp Drive
...
ai-footer-enable-notifications-tooltip = Install the Warp plugin to enable rich agent notifications within Narwarp
...
zap-launch-title = Narwarp is now open-source
zap-launch-description = You, our community, can participate in building Narwarp using an agent-first workflow.
zap-launch-contribute-title = Contribute
zap-launch-contribute-description = Narwarp's client code is now open source. Get started by using the /feedback skill to open an issue, and follow the contribution guidelines here.
zap-launch-contribute-link-text = here
zap-launch-oad-title = Open Automated Development
zap-launch-oad-description = The Narwarp repo is managed by an agent-first local workflow powered by Oz.
...
hoa-finish = Finish
session-config-get-warping = Get Narwarping
...
terminal-warpify-subshell-tooltip = Enable Narwarp shell integration in this session
terminal-use-agent = Use agent
terminal-use-agent-tooltip = Ask the Tusk to assist
terminal-give-control-back-to-agent = Give control back to agent
terminal-resume-agent-tooltip = Ask the Tusk to resume
...
terminal-open-in-warp = Open in Narwarp
terminal-open-conversation-in-warp-tooltip = Open this conversation in the Narwarp desktop app
...
terminal-input-agent-hint-deploy-react-vercel = Narwarp anything e.g. Deploy my React app to Vercel and set up environment variables
terminal-input-agent-hint-debug-python-ci = Narwarp anything e.g. Help me debug why my Python tests are failing in CI
terminal-input-agent-hint-setup-microservice = Narwarp anything e.g. Set up a new microservice with Docker and create the deployment pipeline
terminal-input-agent-hint-fix-node-memory-leak = Narwarp anything e.g. Find and fix the memory leak in my Node.js application
terminal-input-agent-hint-backup-postgres = Narwarp anything e.g. Create a backup script for my PostgreSQL database and schedule it
terminal-input-agent-hint-migrate-mysql-postgres = Narwarp anything e.g. Help me migrate my data from MySQL to PostgreSQL
terminal-input-agent-hint-monitor-aws = Narwarp anything e.g. Set up monitoring and alerts for my AWS infrastructure
terminal-input-agent-hint-build-fastapi = Narwarp anything e.g. Build a REST API for my mobile app using FastAPI
terminal-input-agent-hint-optimize-sql = Narwarp anything e.g. Help me optimize my SQL queries that are running slowly
terminal-input-agent-hint-github-actions = Narwarp anything e.g. Create a GitHub Actions workflow to automatically deploy on merge
terminal-input-agent-hint-redis-cache = Narwarp anything e.g. Set up Redis caching for my web application
terminal-input-agent-hint-kubernetes-pods = Narwarp anything e.g. Help me troubleshoot why my Kubernetes pods keep crashing
terminal-input-agent-hint-bigquery-pipeline = Narwarp anything e.g. Build a data pipeline to process CSV files and load them into BigQuery
terminal-input-agent-hint-ssl-https = Narwarp anything e.g. Set up SSL certificates and configure HTTPS for my domain
terminal-input-agent-hint-refactor-legacy-code = Narwarp anything e.g. Help me refactor this legacy code to use modern design patterns
terminal-input-agent-hint-unit-tests = Narwarp anything e.g. Create unit tests for my authentication service
terminal-input-agent-hint-elk-logs = Narwarp anything e.g. Set up log aggregation with ELK stack for my distributed system
terminal-input-agent-hint-oauth-express = Narwarp anything e.g. Help me implement OAuth2 authentication in my Express.js app
terminal-input-agent-hint-optimize-docker = Narwarp anything e.g. Optimize my Docker images to reduce build times and size
terminal-input-agent-hint-ab-testing = Narwarp anything e.g. Set up A/B testing infrastructure for my web application
...
terminal-model-specs-description = Narwarp's benchmarks for how well a model performs in our harness, the rate at which it consumes credits, and task speed.
...
onboarding-callout-meet-input-title = Meet the Narwarp input
...
onboarding-callout-nl-support-text-prefix = Natural language input is off by default. If enabled, you can type requests in plain English and Narwarp will autodetect queries for the agent. You can always override them using
onboarding-callout-enable-nl-detection = Enable Natural Language Detection
onboarding-callout-new-agent-title = Introducing Narwarp's new agent experience
...
language-widget-secondary = Restart Narwarp for the change to fully take effect.
language-restart-required-title = Language changed
language-restart-required-body = Narwarp's UI language has been updated. Some text will switch immediately, but a full restart is required for the change to take effect everywhere.
...
# Narwarp: settings-section-account removed alongside the Account settings page.
...
settings-section-warp-agent = Tusk
...
# about_page.rs
settings-about-copyright = Copyright 2026 Narwarp
settings-about-automatic-updates-label = Automatic updates
settings-about-automatic-updates-description = When enabled, Narwarp checks for new versions in the background and downloads the installer to a local cache. The currently running Narwarp is not touched until you click "Install now" to launch the installer yourself.
settings-about-update-checking = Checking for updates…
settings-about-update-up-to-date = Narwarp is up to date.
...
settings-about-update-install-hint-macos = The installer will open — drag Narwarp into your Applications folder to finish.
settings-about-update-install-hint-windows = The setup wizard will launch — follow the prompts to finish the upgrade.
settings-about-update-install-hint-linux = The AppImage will be replaced in place and Narwarp will restart.
...
# Narwarp: main_page.rs (Account / version / autoupdate) strings removed alongside
...
# ---- list_page.rs ----
settings-mcp-list-description = Add MCP servers to extend the Tusk's capabilities. MCP servers expose data sources or tools to agents through a standardized interface, essentially acting like plugins. Add a custom server, or use the presets to get started with popular servers.
...
settings-mcp-list-section-shared-by-warp-and-team = Available from Narwarp and { $name }
settings-mcp-list-section-shared-by-warp-and-other-devices = Shared by Narwarp and from other devices
settings-mcp-list-section-shared-from-warp = Shared from Narwarp
...
settings-platform-description = Create and manage API keys to allow local agents to access your Narwarp account.
...
settings-platform-zero-state-description = Create a key to manage external access to Narwarp
settings-platform-create-api-key-description-personal = This API key is tied to your user and can make requests against your Narwarp account.
...
settings-platform-create-api-key-name-placeholder = Narwarp API Key
...
settings-referrals-page-title = Invite a friend to Narwarp
settings-referrals-anonymous-header = Referral program is unavailable in local Narwarp builds
...
settings-referrals-reward-intro = Get exclusive Narwarp goodies when you refer someone*
...
settings-warpify-description-prefix = Configure whether Narwarp attempts to "Warpify" (add support for blocks, input modes, etc) certain shells.
...
settings-warpify-install-ssh-extension-description = Controls the installation behavior for Narwarp's SSH extension when a remote host doesn't have it installed.
...
# Section / sub-headers
settings-ai-warp-agent-header = Tusk
...
settings-ai-command-denylist-description = Regular expressions to match commands that the Tusk should always ask permission to execute.
settings-ai-command-allowlist = Command allowlist
settings-ai-command-allowlist-description = Regular expressions to match commands that can be automatically executed by the Tusk.
...
settings-ai-mcp-allowlist-description = Allow the Tusk to call these MCP servers.
settings-ai-mcp-denylist = MCP denylist
settings-ai-mcp-denylist-description = The Tusk will always ask for permission before calling any MCP servers on this list.
...
settings-ai-base-model-description = This model serves as the primary engine behind the Tusk. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Narwarp may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
settings-ai-show-model-picker-in-prompt = Show model picker in prompt
settings-ai-codebase-context = Codebase Context
settings-ai-codebase-context-description = Allow the Tusk to generate an outline of your codebase that can be used for context. No code is ever stored on our servers.
...
settings-ai-rules-description = Rules help the Tusk follow your conventions, whether for codebases or specific workflows.
...
settings-ai-cloud-agent-computer-use-description = Enable computer use in agent conversations started from the Narwarp app.

# AWS Bedrock
settings-ai-aws-bedrock-toggle = Use AWS Bedrock credentials
settings-ai-aws-bedrock-description = Narwarp loads and sends local AWS CLI credentials for Bedrock-supported models.
settings-ai-aws-bedrock-description-managed = Narwarp loads and sends local AWS CLI credentials for Bedrock-supported models. This setting is managed by your organization.
...
settings-features-ssh-wrapper = Narwarp SSH Wrapper
settings-features-ssh-auto-discovery = Auto-discover SSH hosts
settings-features-receive-desktop-notifications = Receive desktop notifications from Narwarp
...
settings-features-make-default-terminal = Make Narwarp the default terminal
...
settings-features-default-terminal-current = Narwarp is the default terminal
...
settings-features-restart-warp-to-apply = Restart Narwarp for changes to take effect.
...
settings-exec-profile-editor-base-model-desc = This model serves as the primary engine behind the agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Narwarp may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
...
settings-show-blocks-confirm-dialog-text = Are you sure you want to unshare this block?
    ...
    It will no longer be accessible by link and will be permanently deleted from Narwarp servers.
...
settings-appearance-custom-icon-restart-warning = You may need to restart Narwarp for MacOS to apply the preferred icon style.
...
settings-appearance-input-mode-pinned-bottom = Pin to the bottom (Narwarp mode)
...
settings-environments-page-description = Environments define where your ambient agents run. Set one up in minutes via GitHub (recommended), Narwarp-assisted setup, or manual configuration.
...
settings-environments-section-team-default = Provided by Narwarp and this device
settings-environments-section-team-named = Shared by Narwarp and { $team }
...
quit-warning-title-app = Quit Narwarp?
...
keybinding-desc-workspace-left-panel-warp-drive = Left Panel: Narwarp Drive
...
keybinding-desc-workspace-left-panel-ollama-monitor = Left Panel: Ollama Monitor
keybinding-desc-workspace-left-panel-vault-explorer = Left Panel: Vault Explorer
...
keybinding-desc-workspace-toggle-warp-drive = Toggle Narwarp Drive
...
keybinding-desc-workspace-search-drive = Search Narwarp Drive
...
keybinding-desc-workspace-terminate-app = Quit Narwarp
...
keybinding-desc-workspace-export-all-warp-drive-objects = Export all Narwarp Drive objects
...
# AI assistant / agents
keybinding-desc-workspace-toggle-ai-assistant = Toggle Narwarp AI
...
# Narwarp: keybinding-desc-workspace-show-settings-account removed alongside the
...
keybinding-desc-workspace-show-settings-about-menu = About Narwarp
...
keybinding-desc-workspace-view-logs = View Narwarp logs
...
keybinding-desc-workspace-open-zap-launch-modal = [Debug] Open Narwarp Launch Modal
keybinding-desc-workspace-reset-zap-launch-modal-state = [Debug] Reset Narwarp Launch Modal State
...
keybinding-desc-input-show-network-log = Show Narwarp network log
...
keybinding-desc-ai-assistant-close = Close Narwarp AI
keybinding-desc-ai-assistant-focus-terminal-input = Focus Terminal Input From Narwarp AI
keybinding-desc-ai-assistant-restart = Restart Narwarp AI
...
keybinding-desc-terminal-ask-ai-selection = Ask Narwarp AI about Selection
keybinding-desc-terminal-ask-ai-last-block = Ask Narwarp AI about last block
keybinding-desc-terminal-ask-ai = Ask Narwarp AI
...
toggle-suffix-ssh-wrapper = Narwarp SSH wrapper
...
settings-external-editor-choose-layout = Choose a layout to open files in Narwarp
...
settings-external-editor-prefer-markdown = Open Markdown files in Narwarp's Markdown Viewer by default
...
menu-block-open-in-warp = Open in Narwarp
...
menu-block-ask-warp-ai = Ask Narwarp AI
...
menu-input-ask-warp-ai = Ask Narwarp AI
...
slash-cmd-open-skill-desc = Open a skill's markdown file in Narwarp's built-in editor
...
slash-cmd-open-file-desc = Open a file in Narwarp's code editor
...
prompt-editor-warp-prompt-section = Narwarp terminal prompt
...
welcome-tips-command-palette-description = Easily discover everything you can do in Narwarp without your hands leaving the keyboard.
...
welcome-tips-theme-picker-description = Make Narwarp your own by choosing a built-in theme. Or create your own.
...
drive-local-storage-limit-description = Local storage limits are enforced on this device. Remove unused items to create space for new Narwarp Drive objects.
...
workflow-ai-assist-tooltip = Generate a title, descriptions, or parameters with Narwarp AI
...
workspace-left-panel-ollama-monitor = Ollama Monitor
workspace-left-panel-vault-explorer = Vault Explorer
...
wasm-nux-open-desktop-title = Open in Narwarp Desktop?
wasm-nux-open-desktop-detail = Future links will automatically open on desktop.
wasm-nux-open-desktop-confirm = Open in Narwarp
wasm-nux-download-title = Download Narwarp Desktop?
wasm-nux-download-description = Narwarp is the intelligent terminal with AI and your dev team's knowledge built-in.
wasm-nux-learn-more = Learn more
wasm-nux-download-confirm = Download
wasm-nux-object-kind-drive-objects = Narwarp Drive objects
wasm-nux-object-kind-warp-links = Narwarp links
...
auth-override-warning-confirm-title = Delete personal Narwarp Drive objects and preferences?
auth-override-warning-description = It looks like you logged into a Narwarp account through a web browser. If you continue, any personal Narwarp Drive objects and preferences from this anonymous session will be permanently deleted.
...
auth-override-warning-accessibility-help = Narwarp has detected a new login from a web browser. Press escape to cancel and continue using Narwarp without login.
...
auth-needs-sso-link-detail = Click the button below to link your Narwarp account to your SSO provider.
...
auth-offline-first-use-description = You are currently offline. An internet connection is required to use Narwarp for the first time.
auth-offline-first-use-learn-more = Learn more
auth-offline-overlay-title = Using Narwarp Offline
auth-offline-overlay-paragraph-1 = Narwarp can be used offline for local terminal and agent workflows.
...
auth-privacy-settings-help-improve = Help improve Narwarp
auth-privacy-settings-help-improve-description = High-level feature usage data helps Narwarp's product team prioritize the roadmap.
auth-privacy-settings-learn-more = Learn more
auth-privacy-settings-send-crash-reports = Send crash reports
auth-privacy-settings-crash-reports-description = Crash reporting helps Narwarp's engineering team understand stability and improve performance.
...
auth-logout-unsynced-drive-objects-warning = You have { $count } unsynced Narwarp Drive { $count ->
        [one] object
       *[other] objects
...
cli-agent-plugin-codex-install-title = Enable Narwarp Notifications for Codex
cli-agent-plugin-codex-install-subtitle = Update Codex to the latest version, then enable in-focus notifications so Narwarp can display them while you work.
...
cli-agent-plugin-deepseek-install-title = Enable Narwarp Notifications for DeepSeek
...
cli-agent-plugin-claude-install-title = Install Narwarp Plugin for Claude Code
...
cli-agent-plugin-claude-update-title = Update Narwarp Plugin for Claude Code
...
cli-agent-plugin-gemini-install-title = Install Narwarp Plugin for Gemini CLI
...
cli-agent-plugin-gemini-update-title = Update Narwarp Plugin for Gemini CLI
...
cli-agent-plugin-opencode-install-title = Install Narwarp Plugin for OpenCode
...
cli-agent-plugin-opencode-update-title = Update Narwarp Plugin for OpenCode
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = Installing Narwarp SSH Extension... ({ $p }%)
terminal-bootstrapping-installing-warp-ssh-extension = Installing Narwarp SSH Extension...
terminal-bootstrapping-updating-warp-ssh-extension = Updating Narwarp SSH Extension...
...
agent-tip-command-palette = <keybinding> to open the Command Palette and access Narwarp actions and shortcuts.
...
agent-tip-add-context = `@` to add context from files, blocks, or Narwarp Drive objects to your prompt.
...
hoa-welcome-banner-title = Introducing universal agent support: level up any coding agent with Narwarp
...
hoa-feature-native-code-review-description = Send inline comments from Narwarp's code review directly to Claude Code, Codex, or OpenCode
resource-center-whats-new-section = What's New?
resource-center-getting-started-section = Getting Started
resource-center-maximize-warp-section = Maximize Narwarp
...
resource-center-command-palette-description = Access all of Narwarp via the keyboard.
resource-center-set-theme-title = Set your theme
resource-center-set-theme-description = Make Narwarp your own by choosing a theme.
resource-center-custom-prompt-title = Use your custom prompt
resource-center-custom-prompt-description = Set up Narwarp to honor your PS1 setting
resource-center-view-documentation = View documentation
resource-center-integrate-ide-title = Integrate Narwarp with your IDE
resource-center-integrate-ide-description = Configure Narwarp to launch from your most used development tools
resource-center-how-warp-uses-warp-title = How Narwarp uses Narwarp
resource-center-how-warp-uses-warp-description = Learn how Narwarp's engineering team uses their favorite features
...
onboarding-intention-title = Welcome to Narwarp
...
onboarding-ai-feature-warp-agents = Tusks
...
onboarding-agent-title = Customize your Tusk
...
onboarding-agent-disable-warp-agent = Disable Tusk
onboarding-project-title = Open a project
onboarding-project-subtitle = Set up a project to optimize it for coding in Narwarp.
...
onboarding-customize-title = Customize your Narwarp
...
auth-opt-out-line-1 = Narwarp stores onboarding choices locally.
...
auth-local-privacy-note = Narwarp stores onboarding choices locally on this device.
...
auth-login-slide-title-warp-drive = Get started with Narwarp Drive
...
auth-disable-warp-drive = Disable Narwarp Drive
auth-disable-ai-features = Disable AI features
auth-enable-warp-drive = Enable Narwarp Drive
...
auth-disable-warp-drive-confirm-title = Are you sure you want to disable Narwarp Drive?
auth-disable-ai-features-confirm-title = Are you sure you want to disable AI features?
auth-disable-warp-drive-confirm-body = Narwarp Drive lets you save workflows and knowledge across devices and share them with your team. By continuing, you won't have access to the following features:
auth-disable-ai-features-confirm-body = Narwarp is better with AI. By continuing, you won't have access to any of the following features:
...
auth-require-login-ai-collaboration = Local AI features do not require a Narwarp account.
auth-require-login-drive-limit = Narwarp Drive objects are stored locally in Narwarp.
auth-require-login-share = Sharing is unavailable in local Narwarp builds.
auth-welcome-title = Welcome to Narwarp!
auth-sign-up-for-warp = Continue in Narwarp
...
get-started-welcome-title = Welcome to Narwarp
...
terminal-banner-p10k-supported = Powerlevel10k now supports Narwarp!{"  "}
...
terminal-banner-pure-unsupported = Pure is not yet supported in Narwarp. You might consider one of the supported prompts as an alternative.{"  "}
...
settings-import-reset-to-warp-defaults = Reset to Narwarp defaults
...
workspace-notification-permission-denied-toast = Narwarp doesn't have permission to send desktop notifications.
workspace-troubleshoot-notifications-link = Troubleshoot notifications
workspace-plan-synced-to-warp-drive-toast = Plan synced to your Narwarp Drive
workspace-remote-control-link-copied-toast = Remote control link copied.
workspace-update-now = Update now
workspace-update-warp = Update Narwarp
...
workspace-version-deprecation-without-permissions-banner = Some Narwarp features may not work as expected without updating immediately, but Narwarp is unable to perform the update.
workspace-new-version-unable-to-update-banner = A new version is available but Narwarp is unable to perform the update.
workspace-unable-to-launch-new-installed-version = Narwarp was unable to launch the new installed version.
...
terminal-ssh-report-issue-prefix = We are actively working on improving the stability of SSH in Narwarp. Please consider{" "}
...
terminal-agent-mode-setup-title = Optimize Narwarp for this codebase?
...
notifications-banner-permissions-denied-title = Narwarp was denied permissions to send you notifications.
...
tooltip-secrets-not-sent-to-warp-server = *Secrets are not sent to Narwarp's server.
